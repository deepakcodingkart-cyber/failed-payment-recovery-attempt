import {
  ...
  updateRecoveryAfterAttempt,
} from "./recoveryQueries.mjs";
import { executeFallback } from "./fallbackExecutor.mjs";
import { attemptCharge } from "./shopify.mjs";
import { logger } from "./logger.mjs";

export async function processRecoveryMessage(msg) {
  const {
    recoveryId,
    shop_id,
    subscription_id,
  ...
  /* ---------- 1. DB VALIDATION ---------- */
  const state = await validateRecoveryState(recoveryId);
  ...
  if (state.attempt_count !== attemptNumber - 1) {
    logger.warn({ step: "ATTEMPT_MISMATCH", recoveryId, db: state.attempt_count, msg: attemptNumber });
    return;
  }
  ...
  try {
    ...
    await updateRecoveryAfterAttempt({
